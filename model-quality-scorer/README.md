...
- **`pkg/normalize`** (Day 79) — `Score` converts a rubric's raw 0-100 `WeightedScore` output into the 0-1 `normalized_score` a rollup can compare across `task_type`s whose rubrics carry different criteria counts and weights.
- **`pkg/rollup`** (Day 79) — `Query(Window1h | Window24h)` returns the query-time SQL that aggregates `normalized_score` into `(window, model_id, task_type)` buckets (DESIGN.md §6: never a materialized rollup table); `StandardError`/`LowConfidence` apply NOISE-FLOOR.md's 30-sample statistical floor to a bucket's mean.
- **`pkg/decision`** (Day 80) — RouteIQ's weighted-utility routing decision, DESIGN.md §8: `Utility` scalarizes a `Candidate`'s quality (from `FromRollupRow`, wrapping a `rollup.Row`), cost, and latency into one number via tenant-overridable `RoutingWeights`; `Decide` picks the highest-utility candidate, tie-breaking toward the cheaper model within `tieEpsilon`.
